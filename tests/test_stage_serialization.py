...
class NarrationInitCursor(InitCursor):
    ...
    def fetchone(self):
        if self.selected_task:
            self.selected_task = False
            return {'task_id': 'narration-7', 'sub_stage': 'narration', 'task_type': 'narration'}
        return None

    def fetchall(self):
        if self.selected_sentences:
            self.selected_sentences = False
            return [{'line_index': 1, 'sentence_text': '第一行。', 'segment_index': 1}, {'line_index': 2, 'sentence_text': '第二行。', 'segment_index': 1}, {'line_index': 3, 'sentence_text': '第三行。', 'segment_index': 2}]
        return []
...
class DubbingMultiSegmentInitCursor(InitCursor):
    ...
    def fetchone(self):
        if self.selected_task:
            self.selected_task = False
            return {'task_id': self.task_id, 'sub_stage': db.SPEAKER_DUBBING_MULTI_SEGMENT_SUB_STAGE, 'task_type': self.task_type}
...
class StageSerializationTest(unittest.TestCase):
    ...
    def test_ready_segment_query_uses_task_priority_without_translator_gate(self) -> None:
        cursor = FakeCursor()
        with patch.object(db, 'connect', return_value=FakeConnection(cursor)), patch.object(db.task_info, 'merge_into', side_effect=lambda row: row):
    ...
    def test_claim_segment_filters_by_speaker_sub_stage(self) -> None:
        cursor = FakeCursor()
        with patch.object(db, 'connect', return_value=FakeConnection(cursor)), patch.object(db, '_ensure_operator_columns'), patch.object(db, '_operator_value', return_value='MY_HP'):
            self.assertIsNone(db.claim_speaker_segment(12))
        self.assertIn("WHEN vi.task_type = 'narration' THEN %s", cursor.sql)
        self.assertIn('sp.status IN (%s, %s)', cursor.sql)
        self.assertEqual((db.SPEAKER_NARRATION_SUB_STAGE, *db.CHUNK_SPEAKER_TASK_TYPES, db.SPEAKER_DUBBING_MULTI_SEGMENT_SUB_STAGE, db.SPEAKER_MAIN_SUB_STAGE, db.SEGMENT_RUNNING, 'MY_HP', 12, db.SEGMENT_READY, db.READY, db.RUNNING), cursor.params)
        self.assertNotIn('translator', cursor.sql)

    def test_mark_segment_success_records_current_operator(self) -> None:
        cursor = FakeCursor()
        with patch.object(db, 'connect', return_value=FakeConnection(cursor)), patch.object(db, '_operator_value', return_value='MY_HP'):
        ...
        self.assertEqual((db.SEGMENT_SUCCESS, '/tmp/ref.wav', 'minio://ref', '/tmp/tts.wav', 'minio://tts', 'MY_HP', 12), cursor.params)
    ...
    def test_narration_groups_sentence_rows_by_segment_index(self) -> None:
        ...
        self.assertEqual([('narration-7', 0, db.SEGMENT_READY, '第一行。\n第二行。', '第一行。\n第二行。'), ('narration-7', 1, db.SEGMENT_READY, '第三行。', '第三行。')], cursor.params)
    ...
    def test_narration_segment_builder_rejects_non_contiguous_segment_indexes(self) -> None:
        with self.assertRaisesRegex(ValueError, 'segment index is not contiguous'):
            db._build_narration_segments([{'line_index': 1, 'sentence_text': '第一行。', 'segment_index': 1}, {'line_index': 2, 'sentence_text': '第二行。', 'segment_index': 3}])

    def test_narration_segment_builder_requires_first_segment_to_be_one(self) -> None:
        with self.assertRaisesRegex(ValueError, 'segment index is not contiguous'):
            db._build_narration_segments([{'line_index': 1, 'sentence_text': '第一行。', 'segment_index': 2}])
