...
def mark_speaker_segment_success(
    ...
    operator = _operator_value()
    where = "id = %s"
    where_values: list[Any] = [segment_id]
    if attempt_count is not None:
        where += " AND attempt_count = %s AND status = %s"
        where_values.extend([attempt_count, SEGMENT_RUNNING])
    with connect() as conn:
        cur = conn.cursor()
        cur.execute(
            f"""
...
def mark_speaker_segment_failed(segment_id: int, message: str, attempt_count: int | None = None) -> bool:
    with connect() as conn:
        ...
        completed_at = "NOW()" if exhausted else "NULL"
        where = "id = %s"
        values: list[Any] = [status, message, segment_id]
        if attempt_count is not None:
            where += " AND attempt_count = %s AND status = %s"
            values.extend([attempt_count, SEGMENT_RUNNING])
        cur = conn.cursor()
        cur.execute(
            f"""
            UPDATE speaker_segment
        ...
        if recycled_failed_task_ids:
            ...
            cur.execute(
                ...
                WHERE sp.task_id IN ({placeholders})
                  ...
                  AND sp.sub_stage = CASE
                        WHEN EXISTS (
                          SELECT 1 FROM task vi
                          WHERE vi.id = sp.task_id AND vi.task_type = 'narration'
                        ) THEN %s
                        WHEN EXISTS (
                          SELECT 1 FROM task vi
                          WHERE vi.id = sp.task_id AND vi.task_type IN (%s, %s)
                        ) THEN %s
                        WHEN EXISTS (
                          SELECT 1 FROM task vi
                          WHERE vi.id = sp.task_id AND vi.task_type = 'ppt'
                        ) THEN %s
                        ELSE %s
                      END
                  AND sp.status = %s
